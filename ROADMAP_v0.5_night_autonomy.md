...
> 状态：✅ 2026-08-30 以**零代码方案**落地——面板「未来任务」直接建任务（建在平板），实测效果不错，**代码版挂起**。
> 未来若触发「重启代码版的条件」（见文末），再按「复工时的实现清单」动工。
...
## 零代码验证方案 → ✅ 已于 2026-08-30 转正（最终方案）

原计划是睡前用嘴嘱咐（主动 Agent 默认已启用 `add_cron_tools: True`）让机器人自己定任务，带实感再写代码。实际执行时发现面板「未来任务」本身支持 每天/每周/每月/一次性/间隔 五种重复模式，于是升级为更直接的做法：

**主人在平板（君迁的家）的「未来任务」面板手动建夜间任务（如"每天 01:47"），备注写性格引导**（"自己的时间、想干嘛干嘛、主人睡了、留言像字条别刷屏"），君迁半夜醒来自己选择做什么。**实测效果不错。**
...
出现任何一条，再回来按下方「复工时的实现清单」写 `night_autonomy.py`：
1. 嫌每晚**准点醒**太机械，想要随机唤醒时刻（面板任务时刻固定）
2. 闹钟与作息**打架**（它还没睡着闹钟就响 / 起床后残留任务仍触发）——面板任务不感知 circadian 的睡眠状态机（灵活入睡窗口每天在变）
3. 想要**动态小纸条**（醒来时的心情、前几次醒来干了什么）——面板备注是每晚相同的死文案
